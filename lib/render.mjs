...
function requestHref(formUrl, bikeName) {
  // Prefill the request form's Bike field with this bike's name.
  return `${formUrl}?prefill_Bike=${encodeURIComponent(bikeName)}`;
}

// The card's call to action: an SMS link if a phone is configured,
// else a booking-request form link if one is set, else nothing.
function renderCta(bike, opts) {
  if (opts.contactPhone) {
    return `<a class="bike__cta" href="${escapeHtml(smsHref(opts.contactPhone, bike.name))}">Text to book this bike</a>`;
  }
  if (opts.requestFormUrl) {
    return `<a class="bike__cta" href="${escapeHtml(requestHref(opts.requestFormUrl, bike.name))}" target="_blank" rel="noopener">Request this bike →</a>`;
  }
  return '';
}
